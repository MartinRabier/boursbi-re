...
class Market:
    ...
    def plot_prices(self):
        self.ax.clear()
        for stock in self.stocks:
            history_to_display = stock.price_history[-config.DISPLAY_WINDOW_LENGTH:]
        ...
        self.ax.set_xlim(right=x_max+4.75)
        margin_x_position = x_max + 5 
        for stock in self.stocks:
            history_to_display = stock.price_history[-config.DISPLAY_WINDOW_LENGTH:]
            self.ax.text(margin_x_position, history_to_display[-1],f'{history_to_display[-1]:.2f}€',fontsize=config.DISPLAY_SETTINGS["text_fontsize"],verticalalignment='center', horizontalalignment='left', color=stock.color, clip_on=False)  # clip_on=False pour afficher hors limites
